...
- ✅ **文件锁定/解锁**：支持 `svn lock` / `svn unlock`，含远程锁状态检测、强制夺锁与查看锁定信息
- ✅ **统一的 SVN 父级菜单**：右键菜单收纳到一个 **SVN** 子菜单中，按功能分组，简洁不拥挤
- ✅ **自动忽略 svn:externals**：`svn status` 自动加 `--ignore-externals`，提交面板不再显示外部链接目录
...
为协作场景下的二进制资源、配置文件等提供独占编辑保护，避免多人并发修改导致的合并冲突。

#### 使用方式

1. 在 VSCode 资源管理器中**右键单个文件** → 选择以下任一菜单：
   - **SVN: 锁定文件** —— 对当前文件执行 `svn lock`
   - **SVN: 解锁文件** —— 对当前文件执行 `svn unlock`
   - **SVN: 查看锁定信息** —— 查看远程仓库中该文件的锁状态、锁定者、备注与时间
2. 在编辑器中**右键编辑区**也支持以上三个命令；
3. 也可通过命令面板 `Ctrl+Shift+P` 搜索 `SVN: 锁定 / 解锁 / 查看锁定信息`。

#### 功能特性

- **锁定备注**：锁定时支持输入中文备注，便于团队成员了解锁定原因
- **远程锁定检测**：操作前自动通过 `svn info -r HEAD --xml` 查询服务器最新锁状态
- **强制锁定（夺锁）**：发现已被他人锁定时，会弹出确认窗口选择是否使用 `--force` 夺取
- **强制解锁**：解锁时若锁不属于当前用户，会提示是否 `--force` 强制解除
- **当前用户识别**：自动比对已保存的认证用户名，提示锁定者是否为本人
- **认证集成**：lock/unlock 走统一的认证流程，必要时自动使用已保存的凭证或弹窗输入

#### 注意事项

- 锁定/解锁需要远程访问 SVN 服务器，首次使用可能弹出认证对话框
- 强制夺锁/强制解锁会直接覆盖他人锁，请谨慎使用
- 未加入版本控制（`?`）或未提交的本地新增文件无法锁定
